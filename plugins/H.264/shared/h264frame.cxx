...
//  int vopBufferLen;
  ...
  {
    int currentNALLen = NALs[currentNAL].i_payload;//x264
    ...
    memcpy(currentPositionInFrame, NALs[currentNAL].p_payload, currentNALLen);//x264

    if (currentNALLen > 0) 
    {
      _NALs[_numberOfNALsInFrame].length = currentNALLen;
      _NALs[_numberOfNALsInFrame].offset = _encodedFrameLen;
      _NALs[_numberOfNALsInFrame].type = NALs[currentNAL].i_type;//x264
      uint32_t header = 0;
	...
	header = currentPositionInFrame[2] == 1 ? 3 : 4;
      ...
      TRACE_UP(4, "H264\tEncap\tLoaded NAL unit #" << currentNAL << " - type " << NALs[currentNAL].i_type);//x264

      uint8_t* NALptr = NALs[currentNAL].p_payload;//x264
      if ( Trace::CanTraceUserPlane(4) && (NALs[currentNAL].i_type == H264_NAL_TYPE_SEQ_PARAM)) //x264
      {
...
void H264Frame::SetFromFrame (AVCodecContext *Ctx, AVPacket *Pkt) {
//  int vopBufferLen;
  int currentNAL = 0;
  uint8_t* currentPositionInFrame=(uint8_t*) _encodedFrame;
  if (_NALs) free(_NALs);
  ...
  _currentNAL=0;
    ...
    memcpy(currentPositionInFrame, Pkt->data, currentNALLen);//x264
    ...
    if (currentNALLen > 0) 
    {
      ...
      _NALs[_numberOfNALsInFrame].length -= header;
      _NALs[_numberOfNALsInFrame].offset += header;
      TRACE_UP(4, "H264\tEncap\tLoaded NAL unit #" << currentNAL << " - type " << Pkt->flags);//x264

      uint8_t* NALptr = Pkt->data;//x264
...
bool H264Frame::IsSync () {
  ...
  {
    if (_NALs[i].type & AV_PKT_FLAG_KEY)
    {
      return true;
    }
  }
