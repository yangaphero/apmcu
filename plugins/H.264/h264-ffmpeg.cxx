...
H264EncoderContext::H264EncoderContext()
{
  ...
  #endif
  if (_context == NULL) {
    cout << "H264\tEncoder\tFailed to allocate context for Encoder\n";
    return;
  }
  ...
  _context->bit_rate = 20000000;
  _context->width = 352;
  _context->height = 288;
  _context->time_base.num = 1;
  _context->time_base.den = 30;
  ...
  //av_opt_set(_context->priv_data, "rc", "cbr", 0);//cbr的时候每帧前加了sei帧，注意
	...
	av_opt_set(_context->priv_data, "strict_gop", "1", 0);
	av_opt_set(_context->priv_data, "profile", "baseline", 0);
  pict = av_frame_alloc();
  if (!pict) {
    return ;
...
H264EncoderContext::~H264EncoderContext()
{
  ...
  if (_context != NULL)
  {
    ...
    {
    	if(avcodec_close(_context) < 0){
		cout << "[aphero]H264\tencoder\tClose failed \n";
	}else{
		cout << "[aphero]H264\tencoder\tClosed H.264 Encoder \n";
	}
    }
  }
...
{
  //by aphero open encoder
  #if LIBAVCODEC_VERSION_INT > AV_VERSION_INT(53,8,0)
    if (avcodec_open2(_context, _codec, NULL) < 0) {
  #else
    if (avcodec_open(_context, _codec) < 0) {
  #endif
      cout << "[aphero]H264\tEncoder\tFailed to open H.264 Encoder\n";
      return;
    }
    else
    {
      cout << "[aphero]H264\tEncoder\tEncoder successfully opened\n";
    }
//by aphero end
}
...
{
  _txH264Frame->SetMaxPayloadSize(size);//by aphero 
}

void H264EncoderContext::SetTargetBitrate(unsigned rate, unsigned maxBr, unsigned cpb)
{
  // cpb ???
  //_context->bit_rate = rate;     //by aphero
  //_context->max_b_frames = maxBr;//by aphero
  _context->bit_rate = maxBr*1024;  
}

void H264EncoderContext::SetFrameWidth(unsigned width)
{
  _context->width = width;  //by aphero
  
}

void H264EncoderContext::SetFrameHeight(unsigned height)
{
  _context->height = height; //by aphero
}

void H264EncoderContext::SetFrameRate(unsigned rate)
{
  _context->time_base.num = 1; //by aphero
	_context->time_base.den = rate; //by aphero
}

void H264EncoderContext::SetTSTO (unsigned tsto)
{
  //x264->SetTSTO (tsto);//by aphero 不知道怎么改
}

void H264EncoderContext::SetMaxKeyFramePeriod (const char *keyint)
{
  _context->gop_size = 30;// atoi(keyint);//by aphero
}
...
void H264EncoderContext::SetProfileLevel (unsigned profile, unsigned constraints, unsigned level)
{
  ...
  //x264->SetProfileLevel (profileLevel);//by aphero 不知道怎么改
}

void H264EncoderContext::SetQuality (unsigned quality)
{
  //x264->SetQuality (quality);//by aphero 不知道怎么改，原来也未使用
}

void H264EncoderContext::SetThreads(unsigned threads)
{
  _context->thread_count = threads;//by aphero
}
...
*/
//cout << "srcLen:" << srcLen << "width=" << header->width << "height="<< header->height << "\n";
  
  pict->format = AV_PIX_FMT_YUV420P;
	pict->width = header->width;
	pict->height = header->height;
	pict->pts ++;

  pict->data[0] = (uint8_t *)(((unsigned char *)header) + sizeof(frameHeader));
  pict->data[1] = (uint8_t *)((((unsigned char *)header) + sizeof(frameHeader)) + (int)(pict->linesize[0]*header->height));
  pict->data[2] = (uint8_t *)(pict->data[1] + (int)(pict->linesize[1] *header->height/2));
  pict->linesize[0] = header->width;
  pict->linesize[1] = pict->linesize[2] = (int) (header->width / 2);

	ret = avcodec_send_frame(_context, pict);
	if (ret < 0) {
    cout << "avcodec_send_frame failed\n";
    return 1;
	}
